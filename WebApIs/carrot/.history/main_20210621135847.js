...
eventZone.addEventListener('click', event => {
  if(event.target.className === 'fas fa-play'){
    ...
    (function () {
      for(let i =0; i< 11; i++){
      CarrotAndBug.createCarrot();
      CarrotAndBug.createBug(); 
      } 
    })();
    startTimer();
...
class CarrotAndBug {
  ...
  createCarrot(){
    ...
    newCarrot.style.left = Math.random()* 500 + 'px';
    newCarrot.style.right = Math.random()* 500 + 'px';
  ...
  createBug(){
    ...
    newBug.style.left = Math.random()* 500 + 'px';
    newBug.style.right = Math.random()* 500 + 'px';
