...
let second = document.querySelector('.timer p second').innerHTML;
let time;
...
eventZone.addEventListener('click', event => {
  if(event.target.className === 'fas fa-play'){
    ...
    document.querySelector('.timer p second').innerHTML = 10;
    second = 10;
...
function timer(){
  second = second - 1;
  document.querySelector('.timer p second').innerHTML =  document.querySelector('.timer p').innerHTML - 1;
  if(second == 0){
    clearInterval(time);
  }
}