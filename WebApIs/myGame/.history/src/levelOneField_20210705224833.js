...
export class LvOneField{
  ...
  animateFish() {
    for(let wrong of document.querySelectorAll('.wrong')){
      ...
      // let currX = wrong.getBoundingClientRect().x + 'px';
      // let currY = wrong.getBoundingClientRect().y + 'px';
      ...
      let newq = this.makeNewPosition();
      console.log(newq[0].newHeight,newq[1].newHeight);
      console.log(newq[0].newWidth,newq[1].newWidth);
      wrong.animate({ top: newq[0], left: newq[1] }, function(){
        this.animateFish();        
      });
