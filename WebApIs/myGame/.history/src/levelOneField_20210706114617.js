...
export class LvOneField{
  ...
  animateFish() {
    for(let wrong of document.querySelectorAll('.wrong')){
      ...
      let currX = wrong.x - FISH_SIZE;
      let currY = wrong.y - FISH_SIZE;

      // let newX = wrong.getBoundingClientRect().x = Math.random() * 80;
      // let newY = wrong.getBoundingClientRect().y = Math.random() * 45;

      let newX = wrong.getBoundingClientRect().width = Math.floor(Math.random() * currX);
      let newY = wrong.getBoundingClientRect().height = Math.floor(Math.random() * currY);
